...
@test "pkg: ensure fails with missing git repo" {
  run strap::pkg::ensure 'com.github.ultimatestrapdotfiles:straps:0.0.1' #invalid id - no 'straps' repo
  [ "$status" -ne 0 ]
}

@test "pkg: ensure succeeds with HEAD rev" {
  run strap::pkg::ensure 'com.github.ultimatestrapdotfiles:strap'
  #echo "# output: $output" >&3
  [ "$status" -eq 0 ]
  dir="$STRAP_USER_HOME/packages/com/github/ultimatestrapdotfiles/strap/HEAD"
  [ -d "$dir" ]
  rm -rf "$dir"
}
...
@test "pkg: ensure calls git fetch with existing HEAD directory" {
  ...
  local -r parent_dir="$STRAP_USER_HOME/packages/com/github/ultimatestrapdotfiles/strap"
  local -r dir="$parent_dir/HEAD"

  strap::pkg::ensure 'com.github.ultimatestrapdotfiles:strap'
